...
const NavBar: React.FC = () => {
  ...
  const CustomNavLink: React.FC<CustomNavLinkProps> = ({
    ...
    const leftArrow = isLinkActive && (
      <span className="text-[--verdementa] absolute -left-5 top-0 max-lg:hidden">
        &lt;
      </span>
    );

    const rightArrow = isLinkActive && (
      <span className="text-[--verdementa] absolute top-0 -right-10 max-lg:hidden">
        /&gt;
      </span>
    );
          ...
          {navLinks.map((link, index) => (
            <CustomNavLink key={index} link={link.hash} linkEn={link.en}>
              {link.en === activeSection ? (
                <div>
                  <span className="text-[--verdementa] absolute -left-5 top-0">
                    &lt;
                  </span>
